        ...
        <div class="login-inner-contain">
            ...
            <div class="login-form">
                <ul class="login-menu">
                    ...
                    <li class="blue-login"><a href="{{route('customer.login')}}">{{__('Customer Login')}}</a></li>
                    <li class="gray-login"><a href="{{route('vender.login')}}">{{__('Vendor Login')}}</a></li>
                </ul>
                <div class="page-title"><h5><span>{{__('Customer')}}</span> {{__('Login')}}</h5></div>
                ...
                <a href="{{ route('customer.change.langPass') }}" class="text-xs text-primary">{{ __('Forgot Your Password?') }}</a>
                <button type="submit" class="btn-login">{{__('Login')}}</button>
                {{Form::close()}}
            </div>

            <h5 class="copyright-text">
                {{(Utility::getValByName('footer_text')) ? Utility::getValByName('footer_text') :  __('Copyright HRM Troon') }} {{ date('Y') }}
            </h5>
