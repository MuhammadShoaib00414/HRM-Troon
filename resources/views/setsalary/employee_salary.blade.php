...
<br>
  <div class="col-md-12">
    <h2 class="mb-0"><strong>{{__('Earning ')}}</strong></h2>
    <br>
     </div>
    <div class="row">
        <div class="col-12">
            ...
            <div class="row">
                            ...
                            <div class="row">
                                ...
                                @can('create set salary')
                                    <div class="col text-right">
                                        <a href="#" data-url="{{ route('employee.basic.salary',$employee->id) }}" data-size="md" data-ajax-popup="true" data-title="{{__('Set Basic Salary')}}" data-toggle="tooltip" data-original-title="{{__('Basic Salary')}}" class="apply-btn">
                ...
                <br>
                <div class="col-md-12">
                    <h2 class="mb-0"><strong>{{__('Deduction ')}}</strong></h2>
                </div>
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col">
                                    <h6 class="mb-0">{{__('Provident Funds')}}</h6>
                                </div>
                               
                                    <div class="col text-right">
                                        <a href="#" data-url="{{ route('provident.create',$employee->id) }}" data-size="md" data-ajax-popup="true" data-title="{{__('Create Provident Fund')}}" data-toggle="tooltip" data-original-title="{{__('Create Provident Fund')}}" class="apply-btn">
                                            <i class="fa fa-plus"></i>
                                        </a>
                                    </div>
                            </div>
                          ...
                          @if(!$providents->isEmpty())
                            <table class="table table-striped mb-0">
                                ...
                                @foreach ($providents as $provident)
                                    <tr>
                                        <td>{{ !empty($provident->employee())?$provident->employee()->name:'' }}</td>
                                        <td>{{ $provident->employee_amount }}</td>
                                        <td>{{  \Auth::user()->dateFormat($provident->employee_date) }}</td>
                                        <td>{{ $provident->employer_amout }}</td>
                                        <td>{{  \Auth::user()->dateFormat($provident->employee_date) }}</td>

                                        <td class="text-right">
                                            
                                                <a href="#" data-url="{{ URL::to('provident/'.$provident->id.'/edit') }}" data-size="lg" data-ajax-popup="true" data-title="{{__('Edit Provident')}}" class="edit-icon" data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fas fa-pencil-alt"></i></a>


                                                <a href="#" class="delete-icon" data-toggle="tooltip" data-original-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('provident-delete-form-{{$provident->id}}').submit();"><i class="fas fa-trash"></i></a>
                                                {!! Form::open(['method' => 'DELETE', 'route' => ['provident.destroy', $provident->id],'id'=>'provident-delete-form-'.$provident->id]) !!}
                                                {!! Form::close() !!}
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col">
                                    <h6 class="mb-0">{{__('Tax Dedutcion')}}</h6>
                                </div>
                               
                                    <div class="col text-right">
                                        <a href="#" data-url="{{ route('tax.create',$employee->id) }}" data-size="md" data-ajax-popup="true" data-title="{{__('Create Tax')}}" data-toggle="tooltip" data-original-title="{{__('Create Tax ')}}" class="apply-btn">
                                            <i class="fa fa-plus"></i>
                                        </a>
                                    </div>
                            </div>
                        ...
                        <div class="table-responsive">
                          @if(!$taxdeduction->isEmpty())
                            <table class="table table-striped mb-0">
                                ...
                                <tbody>
                                @foreach ($taxdeduction as $tax)
                                    <tr>
                                    <td>{{ !empty($tax->employee())?$tax->employee()->name:'' }}</td>
                                        <td>{{ $tax->tax_amount }}</td>
                                       
                                        <td>{{  \Auth::user()->dateFormat($tax->tax_date) }}</td>

                                        <td class="text-right">
                                            
                                                <a href="#" data-url="{{ URL::to('tax/'.$tax->id.'/edit') }}" data-size="lg" data-ajax-popup="true" data-title="{{__('Edit Tax')}}" class="edit-icon" data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fas fa-pencil-alt"></i></a>


                                                <a href="#" class="delete-icon" data-toggle="tooltip" data-original-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('tax-delete-form-{{$tax->id}}').submit();"><i class="fas fa-trash"></i></a>
                                                {!! Form::open(['method' => 'DELETE', 'route' => ['tax.destroy', $tax->id],'id'=>'tax-delete-form-'.$tax->id]) !!}
                                                {!! Form::close() !!}
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col">
                                    <h6 class="mb-0">{{__('EOBI Amount')}}</h6>
                                </div>
                               
                                    <div class="col text-right">
                                        <a href="#" data-url="{{ route('eobi.create',$employee->id) }}" data-size="md" data-ajax-popup="true" data-title="{{__('EOBI Amount')}}" data-toggle="tooltip" data-original-title="{{__('EOBI Amount')}}" class="apply-btn">
                                            <i class="fa fa-plus"></i>
                                        </a>
                                    </div>
                            </div>
                          ...
                          @if(!$eobis->isEmpty())
                            <table class="table table-striped mb-0">
                                ...
                                @foreach ($eobis as $eobi)
                                    <tr>
                                        <td>{{ !empty($eobi->employee())?$eobi->employee()->name:'' }}</td>
                                        <td>{{ $eobi->employee_amount }}</td>
                                        <td>{{ $eobi->employer_amount }}</td>
                                        <td>{{  \Auth::user()->dateFormat($eobi->eobi_date) }}</td>
                                       
                                        <td class="text-right">
                                            
                                                <a href="#" data-url="{{ URL::to('eobi/'.$eobi->id.'/edit') }}" data-size="lg" data-ajax-popup="true" data-title="{{__('Edit EOBI')}}" class="edit-icon" data-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fas fa-pencil-alt"></i></a>
                                                <a href="#" class="delete-icon" data-toggle="tooltip" data-original-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('eobi-delete-form-{{$eobi->id}}').submit();"><i class="fas fa-trash"></i></a>
                                                {!! Form::open(['method' => 'DELETE', 'route' => ['eobi.destroy', $eobi->id],'id'=>'eobi-delete-form-'.$eobi->id]) !!}
                                                {!! Form::close() !!}
