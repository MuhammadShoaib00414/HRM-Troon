...
@section('page-title')
    {{__('Employee EOBI Status')}}
@endsection
@push('script-page')
@section('action-button')
    <div class="row d-flex justify-content-end">
        <div class="col-xl-5 col-lg-2">
            {{ Form::open(array('url' => array('employee-eobi-status',request()->route('id')),'method'=>'get','id'=>'report_leave')) }}
        ...
        <div class="col-auto my-custom">
            ...
            <a href="{{url('employee-eobi-status',request()->route('id'))}}" class="reset-btn" data-toggle="tooltip"
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table table-striped mb-0" id="report-dataTable">
                            ...
                            <tbody>
                            @foreach($employee_eobis as $eobi)

                                <tr>
                                    <td>{{ \Auth::user()->employeeIdFormat($eobi['id']) }}</td>
                                    <td>{{$eobi['name']}}</td>
                                    <td>{{!empty($eobi['employee_amount']) ? \Auth::user()->priceFormat($eobi['employee_amount']) : '-'}}</td>
                                    <td>{{!empty($eobi['employer_amount']) ? \Auth::user()->priceFormat($eobi['employer_amount']) : '-'}}</td>

                                    <td>{{\Auth::user()->dateFormat($eobi['date'])}}</td>
                                    <td>{{!empty($eobi['total_employee']) ? \Auth::user()->priceFormat($eobi['total_employee']) : '-'}}</td>
                                    <td>
                                        <a href=" {{ url('employee-eobi-status',$eobi['id']) }}" data-size="lg"
                        ...
                        <br>
                            @if($grandTotal)
                                ...
                                <tr>
                                    ...
                                    <td> <strong> {{$grandTotalEmployer}}</strong></td>
                                    <td></td>
