...
@section('action-button')
    <div class="row d-flex justify-content-end">
        ...
        <div class="col-auto my-custom">
            ...
            <a href="{{route('report.leave')}}" class="reset-btn" data-toggle="tooltip"
                    ...
                    <div class="table-responsive py-4">
                        <table class="table table-striped mb-0" id="report-dataTable">
                            ...
                            <tr>
                                ...
                                <th>{{__('Employee')}}</th>
                                <th>{{__('Causal Leave')}}</th>
                                <th>{{__('Sick Leave')}}</th>
                                <th>{{__('Annual Leave')}}</th>
                                <!-- <th>{{__('Total used Leave')}}</th> -->
                                <!-- <th>{{__('Total Additional Leave')}}</th> -->
                                <!-- <th>{{__('Remaining Leave')}}</th>  -->
                            ...
                            <tbody>
                            @foreach($leaveArray as $key => $leave)
           
                                <tr>
                                    <td>{{ \Auth::user()->employeeIdFormat($leave['employee_id']) }}</td>
                                    <td>{{$leave['employee']}}</td>
                                   @if(!empty($leave['casual_Leave']))
                                         <td>{{($leave['casual_Leave']);}}</td>
                                   @else
                                         <td>{{'0'}} </td> 
                                   @endif 

                                   @if(!empty($leave['sick_leave']))
                                         <td>{{($leave['sick_leave']);}}</td>
                                   @else
                                         <td>{{'0'}} </td> 
                                   @endif

                                   @if(!empty($leave['annual_leave']))
                                         <td>{{($leave['annual_leave']);}}</td>
                                   @else
                                          <td>{{'0'}} </td> 
                                   @endif

                                   <!-- @if(!empty($leave['additional_leave']))
                                         <td>{{($leave['additional_leave']);}}</td>
                                   @else
                                         <td>{{'0'}} </td> 
                                   @endif -->

                                   <!-- @if(!empty($leave['Remaining Leave']))
                                         <td>{{($leave['additional_leave']);}}</td>
                                   @else
                                         <td>{{'0'}} </td> 
                                   @endif  -->
<!-- 
                                   @if(!empty($leave['remaing_leave']))
                                         <td>{{($leave['remaing_leave']);}}</td>
                                   @else
                                         <td>{{'0'}} </td> 
                                   @endif -->
