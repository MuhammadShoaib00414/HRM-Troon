        ...
        @foreach($clients as $client)
            <div class="col-lg-3 col-sm-6 col-md-4">
                <div class="card profile-card">
                    ...
                    <h4 class="h4 mb-0 my-2"><a href="{{route('clients.show',$client->id)}}">{{$client->name}}</a></h4>
                    <div class="sal-right-card">
                        <span class="badge badge-pill badge-blue">{{$client->email}}</span>
                    </div>
                    <small class="my-2" data-toggle="tooltip" data-placement="bottom" data-original-title="{{__('Last Login')}}">{{ (!empty($client->last_login_at)) ? $client->last_login_at : '-' }}</small>
