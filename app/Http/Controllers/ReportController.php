...
class ReportController extends Controller
{
    ...
    public function leave(Request $request)
    {
        if(\Auth::user()->can('manage report'))
        {
            ...
            $employees->with(['leaves' => function($q) {
                $q->with(['leaveType' => function($leaveType) { 
                    $leaveType->select('id', 'title', 'days');
                 }]);
                $q->selectRaw("employee_id, total_leave_days, leave_type_id")->where('leave_type_id','1')->orWhere('leave_type_id','3')
                ->orWhere('leave_type_id','8')->groupBy('employee_id', 'leave_type_id')->selectRaw('sum(total_leave_days) as total_leave_days');
            }]);
            ...
            foreach($employees as $employee)
            {
                ...
                $getLeavesArray = $employee->leaves->toArray();
                $getAllLeavesTypes = LeaveType::all()->toArray();
                // dd($getAllLeavesTypes);
                $employeeLeave['leaves']    =  $getLeavesArray; //$employee->leaves->toArray();
               
                /*if (count($employeeLeave['leaves'])) {
                    foreach ($employeeLeave['leaves'] as $key => $leaf) {
                        $leaveType = LeaveType::where('id', '!=', $leaf['leave_type']['id'])->get()->toArray();
                        $employeeLeave['leaves'][$key]['leave_type'] = $leaveType;
                    }
                } else {
                    $employeeLeave['leaves'][0]['leave_type'] = LeaveType::all()->toArray();
                }*/
                ...
                $leaves[] =  collect($employeeLeave)->toArray();
            }
            $leaveArray = Array();
            foreach($leaves as $leaveKey => $leave){
                $leaveArray[$leaveKey]['id'] = $leave['id'];
                $leaveArray[$leaveKey]['employee_id'] = $leave['employee_id'];
                $leaveArray[$leaveKey]['employee'] = $leave['employee'];
                $leaveArray[$leaveKey]['approved'] = $leave['approved'];
                $leaveArray[$leaveKey]['reject'] = $leave['reject'];
                $leaveArray[$leaveKey]['pending'] = $leave['pending'];
                foreach($leave['leaves'] as $totalleaveKey => $totalleave) {
                    if ($totalleave['leave_type']['title'] =="Casual Leave"){
                        $leaveArray[$leaveKey]['casual_Leave'] = $totalleave['leave_type']['days']."(".$totalleave['total_leave_days'].")";
                        $leaveArray[$leaveKey]['additional_leave'] = abs($totalleave['leave_type']['days']-$totalleave['total_leave_days']);
                        $data =  DB::table("leaves")->where('employee_id', $leave['id'])->sum('total_leave_days');
                    } 
                    if ($totalleave['leave_type']['title'] =="Sick Leave"){
                        $leaveArray[$leaveKey]['sick_leave'] = $totalleave['leave_type']['days']."(".$totalleave['total_leave_days'].")";
                        $data =  DB::table("leaves")->where('employee_id', $leave['id'])->sum('total_leave_days');
                        $leaveArray[$leaveKey]['additional_leave'] = $data;
                    }
                    if ($totalleave['leave_type']['title'] =="Annual Leave"){
                        $leaveArray[$leaveKey]['annual_leave'] = $totalleave['leave_type']['days']."(".$totalleave['total_leave_days'].")";
                        $data =  DB::table("leaves")->where('employee_id', $leave['id'])->sum('total_leave_days');
                        $leaveArray[$leaveKey]['additional_leave'] =$data;
                    }
             }
        ...
        //    dd($leaveArray);
            ...
            return view('report.leave', compact('department', 'branch', 'leaves','leaveArray', 'filterYear', 'filter'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function employeeLeave(Request $request, $employee_id, $status, $type, $month, $year)
    {
        ...
        if(\Auth::user()->can('manage report'))
        {
            ...
            $leaveTypes = LeaveType::where('id', \Auth::user()->creatorId())->get();
            if($type == 'yearly')
            {
            ...
            return view('report.leaveShow', compact('leaves', 'leaveData','leaveTypes'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function payroll(Request $request)
    {
        ...
        if(\Auth::user()->can('manage report'))
        {
            ...
            $totalBasicSalary = $totalNetSalary = $totalAllowance = $totalCommision = $totalLoan = 
            $totalSaturationDeduction = $totalOtherPayment = $totalOverTime = $totaleobiPayment= $totaltaxPayment= 
            $totalprovidentPayment = 0;
            ...
            foreach($payslips as $payslip)
            {
                ...
                foreach($eobis as $eobiamount)
                {

                    $totaleobiPayment += $eobiamount->employee_amount;
                }

                $taxDeduction = json_decode($payslip->tax);

//                dd($taxDeduction);

                foreach($taxDeduction as $taxPayment)
                {

                    $totaltaxPayment += $taxPayment->tax_amount;
                }

//                $totalprovidentPayment = json_decode($payslip->provident);
//                foreach($totalprovidentPayment as $providentPayment)
//                {
//                    $totalprovidentPayment += $providentPayment->employer_amout;
//                }
            ...
            $filterData['totaleobiPayment']         = $totaleobiPayment;
            $filterData['totaltaxPayment']          = $totaltaxPayment;
            $filterData['totalprovidentPayment']    = $totalprovidentPayment;
