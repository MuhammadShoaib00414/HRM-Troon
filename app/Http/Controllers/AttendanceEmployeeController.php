...
{
    public function addAttandanceFromMachine() {
      
        $zk = new ZKTeco('192.168.0.50');
        $zk->connect();
   
        $zk->disableDevice();
        if ($zk->connect()){
            $getAttendances = $zk->getAttendance();
            $getUser = $zk->getUser();
            $getAttendance = $zk->getAttendance();
            date_default_timezone_set("Asia/Karachi");
            $getTime =$zk->setTime(date('Y-m-d H:i:s'));
            $getUser = $zk->getUser();
            DB::table('attandance_data')->truncate();
            $getAttendance = array_reverse($getAttendances, true);

            foreach($getAttendance as $data){
                $startTime  = Utility::getValByName('company_start_time');
                $endTime    = Utility::getValByName('company_end_time');
                if($data['type'] == '0'){
                    $checkin = \Auth::user()->timeFormat($data['timestamp']);
                    $checkindiff = date("H:i:s", strtotime($data['timestamp']));

                    $time1 = strtotime($startTime);  // 2012-12-06 23:56
                    $time2 = strtotime($checkin);  // 2012-12-06 00:21
                    $late =  ($time1 - $time2)/60;
                ...
                $employeeAttandancedata = new AttandanceData;
                $employeeAttandancedata->user_id   = $data['id'];
                $employeeAttandancedata->employee_id   = $data['id'];
                $employeeAttandancedata->username   = $data['id'];
                $employeeAttandancedata->status   = 'present';
                $employeeAttandancedata->check_in   =  $checkin;
                $employeeAttandancedata->check_out   = $checkin;
                // $employeeAttandancedata->late   = $total_late;
                // $employeeAttandancedata->overtime	   = $total_late;
                // $employeeAttandancedata->early_leaving   = $total_late;
                $employeeAttandancedata->company_start_time   =  $startTime;
                $employeeAttandancedata->company_end_time   =   $endTime;
                $employeeAttandancedata->type   = $data['type'];
                $employeeAttandancedata->date   = date('Y-m-d', strtotime($data['timestamp']));
                $employeeAttandancedata->save();
    ...
    public function index(Request $request)
    {
        ...
        $filterYear = AttandanceData::select(
            'date'
        )
            ->groupBy(DB::raw("YEAR(date)"))
            ->get()
            ->toArray();
            $zk = new ZKTeco('192.168.0.50','4370');
            $zk->connect();
            // dd($zk->connect());exit;
            $zk->enableDevice();
            $time = $zk->getTime();
            ...
            $getAttendance = $zk->getAttendance();
            date_default_timezone_set("Asia/Karachi");
            $setTime =$zk->setTime(date('Y-m-d H:i:s'));
            $getTime =$zk->getTime();
            // dd($getTime);
            $getUser = $zk->getUser();
            ...
            //     $Employee->save();
            //   }
            DB::table('attendance_employees')->truncate();
            $getAttendance = array_reverse($getAttendance, true);
           foreach($getAttendance as $data){
               $startTime  = Utility::getValByName('company_start_time');
               $endTime    = Utility::getValByName('company_end_time');
              if($data['type'] == '0'){
               ...
               $employeeAttandancedata = new AttendanceEmployee;
               $employeeAttandancedata->employee_id   = $data['id'];
               $employeeAttandancedata->status   = 'Present';
               $employeeAttandancedata->date   = date('Y-m-d', strtotime($data['timestamp']));
               $employeeAttandancedata->clock_in   =  $checkin;
               $employeeAttandancedata->clock_out   = $checkin;
               $employeeAttandancedata->type   = $data['type'];
              $employeeAttandancedata->created_by    = \Auth::user()->creatorId();
...
//                    DB::enableQueryLog();
                    $getAllemployeeAttandanceData=  AttendanceEmployee::

                    whereMonth('date',$month)->get()->toArray();
//                    dd(DB::getQueryLog());
                    return view('attendance.index', compact('filterYear','attendanceEmployee', 'branch', 'department','getAttendance','getUser','getAllemployeeAttandanceData'));
                ...
                else
                {
                    ...
                    if ($request->type == 'year'){
                        ...
                        $getAllemployeeAttandanceData=  AttendanceEmployee::
                        where('date',(Carbon::now()->format('Y-m-d')))->get()->toArray();
                    }
//                    dd($year);
//                    $getAllemployeeAttandanceData=  AttandanceData::
//                    whereYear('date','=',$request->year)->get()->toArray();
//                    $getAllemployeeAttandanceData=  AttandanceData::
//                    whereYear('date',$year)->get()->toArray();
