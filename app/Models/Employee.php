<?php

namespace App\Models;

use Illuminate\Database\Eloquent\Model;

class Employee extends Model
{
    protected $fillable = [
        'user_id',
        'name',
        'dob',
        'gender',
        'phone',

        'father_name',
        'mother_name',
        'marital_status',
        'wife_name',
        'wife_cnic',
        'wife_dob',
        'children_name',
        'children_dob',
        'children_gender',
        
        'emergency_name',
        'emergency_relation',
        'emergency_number',
        'blood_group',
        'emergency_address',
        'professional_start_date',
        'professional_end_date',
        'insititution_name',
        'educational_start_date',
        'educational_end_date',
        'name_degree',
        'specialization',
        'degree_level',
        'degree_copy',
        'employer_name',
        'pro_designation',
        'educational_start_date',
        'educational_end_date',
        'experience_letter',
        'reference_name',
        'refeance_number',
        'refeance_email',
        'profile_image',
        'device_name',
		'device_details',
		'facebook',
		'twitter',
		'google',
		'linkedin',
		'province',
		'city',
		'zip_code',
		'present_address',
		'permanent_address',
		'basic_salary',
		'hourly_rate',
		'salay_type',
		'leave_type',
		'contract_start_date',
		'contract_end_date',
		'reporting_to',
		'office_time',
		'benefits_details',
		'employment_status',
		'contract_department',
		'contract_designation',
		'contract_copy',
        'address',
        'salaries_type',

        'email',
        'password',
        'employee_id',
        'branch_id',
        'department_id',
        'designation_id',
        'company_doj',
        'documents',
        'account_holder_name',
        'account_number',
        'bank_name',
        'bank_identifier_code',
        'branch_location',
        'tax_payer_id',
        'salary_type',
        'salary',
        'created_by',
        'lname','fname','status', 'role','personal_email'
		,'official_email','religion','nationality_id','citizenship_id','cnic','reference'
        
    ];

    public function documents()
    {
        return $this->hasMany('App\Models\EmployeeDocument', 'employee_id', 'employee_id')->get();
    }

    public function salary_type()
    {
        return $this->hasOne('App\Models\PayslipType', 'id', 'salary_type')->pluck('name')->first();
    }

    public function get_net_salary()
    {

        //allowance
        $allowances      = Allowance::where('employee_id', '=', $this->id)->get();
        $total_allowance = 0;
        foreach($allowances as $allowance)
        {
            $total_allowance = $allowance->amount + $total_allowance;
        }

        //commission
        $commissions      = Commission::where('employee_id', '=', $this->id)->get();
        $total_commission = 0;
        foreach($commissions as $commission)
        {
            $total_commission = $commission->amount + $total_commission;
        }

        //Loan
        $loans      = Loan::where('employee_id', '=', $this->id)->get();
        $total_loan = 0;
        foreach($loans as $loan)
        {
            $total_loan = $loan->amount + $total_loan;
        }

        //Saturation Deduction
        $saturation_deductions      = SaturationDeduction::where('employee_id', '=', $this->id)->get();
        $total_saturation_deduction = 0;
        foreach($saturation_deductions as $saturation_deduction)
        {
            $total_saturation_deduction = $saturation_deduction->amount + $total_saturation_deduction;
        }

        //OtherPayment
        $other_payments      = OtherPayment::where('employee_id', '=', $this->id)->get();
        $total_other_payment = 0;
        foreach($other_payments as $other_payment)
        {
            $total_other_payment = $other_payment->amount + $total_other_payment;
        }

        //Overtime
        $over_times      = Overtime::where('employee_id', '=', $this->id)->get();
        $total_over_time = 0;
        foreach($over_times as $over_time)
        {
            $total_work      = $over_time->number_of_days * $over_time->hours;
            $amount          = $total_work * $over_time->rate;
            $total_over_time = $amount + $total_over_time;
        }

            $providents   = Provident::where('employee_id', '=',  $this->id)->get();
            $total_provident_payment = 0;
            foreach($providents as $provident_payment)
            {
                $total_provident_payment = $provident_payment->employee_amount + $total_provident_payment;
               
            }
           
   // $eobis   = EOBI::where('employee_id', '=',  $this->id)->get();
            // $total_eobi_payment = 0;
            // foreach($eobis as $eobi_payment)
            // {
            //     $total_eobi_payment = $eobi_payment->eobi_amount + $total_eobi_payment;
               
            // }
            // $taxDeduction      = TaxDeduction::where('employee_id', '=', $this->id)->get();
            // $total_tax = 0;
            // foreach($taxDeduction as $tax)
            // {
            //     $total_tax = $tax->tax_amount + $total_tax;
            // }

        //Net Salary Calculate
        $advance_salary = $total_allowance + $total_commission - $total_loan - $total_saturation_deduction - $total_provident_payment + $total_other_payment + $total_over_time;
        // dd($advance_salary);
        $employee       = Employee::where('id', '=', $this->id)->first();

        $net_salary     = (!empty($employee->salary) ? $employee->salary : 0) + $advance_salary;

        return $net_salary;

    }

    public static function allowance($id)
    {

        //allowance
        $allowances      = Allowance::where('employee_id', '=', $id)->get();
        $total_allowance = 0;
        foreach($allowances as $allowance)
        {
            $total_allowance = $allowance->amount + $total_allowance;
        }

        $allowance_json = json_encode($allowances);

        return $allowance_json;

    }

    public static function commission($id)
    {
        //commission
        $commissions      = Commission::where('employee_id', '=', $id)->get();
        $total_commission = 0;
        foreach($commissions as $commission)
        {
            $total_commission = $commission->amount + $total_commission;
        }
        $commission_json = json_encode($commissions);

        return $commission_json;

    }

    public static function loan($id)
    {
        //Loan
        $loans      = Loan::where('employee_id', '=', $id)->get();
        $total_loan = 0;
        foreach($loans as $loan)
        {
            $total_loan = $loan->amount + $total_loan;
        }
        $loan_json = json_encode($loans);

        return $loan_json;
    }

    public static function saturation_deduction($id)
    {
        //Saturation Deduction
        $saturation_deductions      = SaturationDeduction::where('employee_id', '=', $id)->get();
        $total_saturation_deduction = 0;
        foreach($saturation_deductions as $saturation_deduction)
        {
            $total_saturation_deduction = $saturation_deduction->amount + $total_saturation_deduction;
        }
        $saturation_deduction_json = json_encode($saturation_deductions);

        return $saturation_deduction_json;

    }

    public static function other_payment($id)
    {
        //OtherPayment
        $other_payments      = OtherPayment::where('employee_id', '=', $id)->get();
        $total_other_payment = 0;
        foreach($other_payments as $other_payment)
        {
            $total_other_payment = $other_payment->amount + $total_other_payment;
        }
        $other_payment_json = json_encode($other_payments);

        return $other_payment_json;
    }

    public static function overtime($id)
    {
        //Overtime
        $over_times      = Overtime::where('employee_id', '=', $id)->get();
        $total_over_time = 0;
        foreach($over_times as $over_time)
        {
            $total_work      = $over_time->number_of_days * $over_time->hours;
            $amount          = $total_work * $over_time->rate;
            $total_over_time = $amount + $total_over_time;
        }
        $over_time_json = json_encode($over_times);

        return $over_time_json;
    }
    public static function EOBI($id)
    {
        //OtherPayment
        $eobis      = EOBI::where('employee_id', '=', $id)->get();
        $total_other_payment = 0;
        foreach($eobis as $other_payment)
        {
            $total_other_eobi = $other_payment->eobi_amount + $total_other_payment;
        }
        $other_eobi_json = json_encode($eobis);

        return $other_eobi_json;
    }

    public static function TAX($id)
    {
        //OtherPayment
        $taxDeduction      = TaxDeduction::where('employee_id', '=', $id)->get();
        $total_taxDeduction = 0;
        foreach($taxDeduction as $other_payment)
        {
            $total_other_tax = $other_payment->tax_amount + $total_taxDeduction;
        }
        $other_tax_json = json_encode($taxDeduction);

        return $other_tax_json;
    }

    public static function Provident($id)
    {
        //OtherPayment
        $providents     = Provident::where('employee_id', '=', $id)->get();
        $total_other_payment = 0;
        foreach($providents as $other_payment)
        {
            $total_other_eobi = $other_payment->employee_amount + $total_other_payment;
        }
        $other_provident_json = json_encode($providents);

        return $other_provident_json;
    }

    
    public static function employee_id()
    {
        $employee = Employee::latest()->first();

        return !empty($employee) ? $employee->id + 1 : 1;
    }

    public function branch()
    {
        return $this->hasOne('App\Models\Branch', 'id', 'branch_id');
    }

    public function department()
    {
        return $this->hasOne('App\Models\Department', 'id', 'department_id');
    }

    public function designation()
    {
        return $this->hasOne('App\Models\Designation', 'id', 'designation_id');
    }

    public function salaryType()
    {
        return $this->hasOne('App\Models\PayslipType', 'id', 'salary_type');
    }

    public function user()
    {
        return $this->hasOne('App\Models\User', 'id', 'user_id');
    }

    public function paySlip()
    {
        return $this->hasOne('App\Models\PaySlip', 'id', 'employee_id');
    }

    public function leaves()
    {
        return $this->hasMany('App\Models\Leave', 'employee_id');
    }


    public function present_status($employee_id, $data)
    {
        return AttendanceEmployee::where('employee_id', $employee_id)->where('date', $data)->first();
    }

    /**
     * Get the user that owns the phone.
     */
    public function employeeUser()
    {
        return $this->belongsTo(Employee::class, 'employee_id');
    }





}
