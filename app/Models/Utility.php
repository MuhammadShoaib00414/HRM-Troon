...
',
    ...
    public static function employeePayslipDetail($employeeId,$month)
    {
        ...
        $earning['allowance']         = Payslip::where('employee_id', $employeeId)->get();
        $earning['totalPayslips']    = Payslip::where('employee_id', $employeeId)->groupBy('employee_id')->get()->sum('basic_salary');
        ...
        $deduction['provident']      = Provident::where('employee_id', $employeeId)->get();
        $deduction['totalProvident'] = Provident::where('employee_id', $employeeId)->get()->sum('employee_amount');
        
        $deduction['taxDeduction']      = TaxDeduction::where('employee_id', $employeeId)->get();
        $deduction['totalTaxDeduction'] = TaxDeduction::where('employee_id', $employeeId)->get()->sum('tax_amount');
      

        $getCurrentDate  = explode('-',$month);
        $deduction['eobis'] = EOBI::where('employee_id', $employeeId)->whereMonth('eobi_date',$getCurrentDate[1])->whereYear('eobi_date',$getCurrentDate[0])->get();

        $deduction['totalEobi'] = EOBI::where('employee_id', $employeeId)->get()->sum('eobi_amount');
       
        $payslip['earning']        = $earning;
        $payslip['totalEarning']   = $earning['totalPayslips']+$earning['totalAllowance'] + $earning['totalCommission'] + $earning['totalOtherPayment'] + $earning['totalOverTime'];

        $payslip['deduction']      = $deduction;
        $payslip['totalDeduction'] =   $deduction['totalTaxDeduction'] + $deduction['totalEobi'] +   $deduction['totalProvident'] + $deduction['totalLoan'] + $deduction['totalDeduction'];

        return $payslip;
    }

    public static function employeePayslipDetailMultiple($employeeId)
    {
        ...
        // DB::enableQueryLog();
        $earning['allowance'] = Payslip::where('employee_id', $employeeId)
            ->where('salary_month', '>=', $startDate)
            ->where('salary_month', '<=', $endDate)->get()->toArray();
        // $query = DB::getQueryLog();
        //        $query = end($query);
        //            dd($query);
// DB::enableQueryLog();
        $earning['totalPayslips'] = Payslip::where('employee_id', $employeeId)
            ->where('salary_month', '>=', $startDate)
            ->where('salary_month', '<=', $endDate)
            ->groupBy('employee_id')->get()->sum('basic_salary');
//         $query = DB::getQueryLog();
//                $query = end($query);
//                    dd($query);
//        dd($earning['totalPayslips']);
        //$earning['allowance'] = Allowance::where('employee_id', $employeeId)->get();
        $earning['totalAllowance'] = Allowance::where('employee_id', $employeeId)->get()->sum('amount');
        $earning['commission'] = Commission::where('employee_id', $employeeId)->get()->toArray();
        $earning['totalCommission'] = Commission::where('employee_id', $employeeId)->get()->sum('amount');

        $earning['otherPayment'] = OtherPayment::where('employee_id', $employeeId)->get()->toArray();
        $earning['totalOtherPayment'] = OtherPayment::where('employee_id', $employeeId)->get()->sum('amount');
        $earning['overTime'] = Overtime::select('id', 'title')->selectRaw('number_of_days * hours* rate as amount')->where('employee_id', $employeeId)->get()->toArray();
        $earning['totalOverTime'] = Overtime::selectRaw('number_of_days * hours* rate as total')->where('employee_id', $employeeId)->get()->sum('total');

        $deduction['loan'] = Loan::where('employee_id', $employeeId)->get()->toArray();

        $deduction['totalLoan'] = Loan::where('employee_id', $employeeId)->get()->sum('amount');
        $deduction['deduction'] = SaturationDeduction::where('employee_id', $employeeId)->get()->toArray();
        $deduction['totalDeduction'] = SaturationDeduction::where('employee_id', $employeeId)->get()->sum('amount');

        $deduction['provident'] = Provident::where('employee_id', $employeeId)->get()->toArray();
        $deduction['totalProvident'] = Provident::where('employee_id', $employeeId)->get()->sum('employee_amount');

        $deduction['taxDeduction'] = TaxDeduction::where('employee_id', $employeeId)->get()->toArray();
        $deduction['totalTaxDeduction'] = TaxDeduction::where('employee_id', $employeeId)->get()->sum('tax_amount');

        $deduction['eobis'] = EOBI::where('employee_id', $employeeId)->get()->toArray();
        // dd($deduction['eobis']);
        $deduction['totalEobi'] = EOBI::where('employee_id', $employeeId)->get()->sum('eobi_amount');
